...
public class DBConnectionHandler extends SQLiteOpenHelper {
   ...
   //    db.execSQL("DROP TABLE " + PARKING_FAVORITES_TABLE_NAME);
        db.execSQL(" CREATE TABLE IF NOT EXISTS "+ STREET_CLEANING_TABLE_NAME+"(WeekDay VARCHAR, RightLeft VARCHAR, Corridor VARCHAR, FromHour VARCHAR, ToHour VARCHAR, Holidays CHAR, Week1OfMonth CHAR, Week2OfMonth CHAR, Week3OfMonth CHAR, Week4OfMonth CHAR, Week5OfMonth CHAR, LF_FADD NUMBER, LF_TOADD NUMBER, RT_TOADD NUMBER, RT_FADD NUMBER, STREETNAME VARCHAR, ZIP_CODE NUMBER, NHOOD VARCHAR);");
        ...
        if(check_TableIsNull(db,STREET_CLEANING_TABLE_NAME)) {
    ...
    public void onUpgrade(SQLiteDatabase db, int OldVersion, int newVersion) {
        db.execSQL("DROP TABLE " +STREET_CLEANING_TABLE_NAME);
        db.execSQL("DROP TABLE " + PARKING_FAVORITES_TABLE_NAME);
        onCreate(db);
    }
    ...
    public ArrayList<StreetCleaningDataBean> getRequiredAddress(Number SUBSTREET, String STREETNAME, Number ZIP_CODE) {
        ...
        String selectQuery = "SELECT * FROM "+STREET_CLEANING_TABLE_NAME+" where STREETNAME='"+ STREETNAME +"'and ZIP_CODE="+ZIP_CODE
